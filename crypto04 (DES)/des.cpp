...
// изменить бит
ulong64_single change_bit_64(ulong64_single x)
{
	ulong64_single y;
	ulong pos = 1;
	y = 1;
...
// разбить 64-битный блок на два 32-битных
...
// объединить два 32-битных блока в один 64-битный
ulong64_single convert_to_ulong64_single(ulong64 x)
{
	ulong64_single b = 0;
	b += x.h;
	b <<= 32;
...
// извлечь n-битный блок
ulong64_single get_block_custom(uchar *src, ulong offset, ulong blocklen)
{
	ulong64_single block;		// 64-битный блок
	ulong bit;					// бит
	block = 0;
...
// записать n-битный блок
void put_block_custom(uchar *dst, ulong offset, ulong64_single block, ulong blocklen)
{
	bool b = false;
	offset += blocklen;
	for (ulong i = 0; i < blocklen; i++)
	{
		b = block % 2 == 1;
		offset--;
		PUT_BIT_BE(b, dst, offset / 8, offset % 8);
		block >>= 1;
	}
}
...
{
	ulong64 block = { 0, 0 };				// обрабатываемый блок
	ulong64 keys[16];						// 
	des_create_keys(key, keys);				// генерация ключей
	for (ulong i = 0; i < srclen; i = i + 8)
	{
		GET_ULONG_BE(block.l, src, i);		// младшие 4 байта
		GET_ULONG_BE(block.h, src, i + 4);	// старшие 4 байта
		block = des_encrypt_block(block, keys);	// шифрование
		PUT_ULONG_BE(block.l, dst, i);		// 
		PUT_ULONG_BE(block.h, dst, i + 4);	// результат
	}
	/*std::string temp((char*)dst);
	temp = temp.substr(0, srclen);
	dst = (uchar*)temp.c_str();*/
	return 1;
}
ulong des_decrypt_ecb(uchar *src, ulong srclen, uchar *dst, ulong64 key)
{
	...
	for (ulong i = 0; i < srclen; i = i + 8)
	{
		GET_ULONG_BE(block.l, src, i);		// младшие 4 байта
		GET_ULONG_BE(block.h, src, i + 4);	// старшие 4 байта
		//if (i == 8) block.h = change_bit_32(block.h);	// изменение бита
		block = des_decrypt_block(block, keys);	// дешифрование
		PUT_ULONG_BE(block.l, dst, i);		// 
		PUT_ULONG_BE(block.h, dst, i + 4);	// результат
	}
	return 1;
...
{
	ulong64 block = { 0, 0 };				// обрабатываемый блок 1
	ulong64 T_block = iv;					// обрабатываемый блок 2
	ulong64 keys[16];						// 
	des_create_keys(key, keys);				// генерация ключей
	...
	for (ulong i = 0; i < srclen; i = i + 8)
	{
		...
		GET_ULONG_BE(block.h, src, i + 4);	// старшие 4 байта
		T_block.l ^= block.l;				// 
		T_block.h ^= block.h;				// сложение по модулю 2

		T_block = des_encrypt_block(T_block, keys);	// шифрование
		PUT_ULONG_BE(T_block.l, dst, i);		// 
		PUT_ULONG_BE(T_block.h, dst, i + 4);	// результат
	}
	return 1;
}
ulong des_decrypt_cbc(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	ulong64 block1 = { 0, 0 };				// обрабатываемый блок 1
	ulong64 block2 = { 0, 0 };				// обрабатываемый блок 2
	ulong64 mem = iv;						// "предыдущий" блок
	ulong64 keys[16];						// 
	des_create_keys(key, keys);				// генерация ключей
	
	for (ulong i = 0; i < srclen; i = i + 8)
	{
		GET_ULONG_BE(block1.l, src, i);		// младшие 4 байта
		GET_ULONG_BE(block1.h, src, i + 4);	// старшие 4 байта
		//if (i == 8) block1.h = change_bit_32(block1.h);
		block2 = mem;
		mem = block1;
		block1 = des_decrypt_block(block1, keys);	// дешифрование
		
		block2.l ^= block1.l;				// 
		block2.h ^= block1.h;				// сложение по модулю 2
		PUT_ULONG_BE(block2.l, dst, i);		// 
		PUT_ULONG_BE(block2.h, dst, i + 4);	// результат
	}
	return 1;
}
...
{
	ulong64_single block = 0;	// обрабатываемый блок 1
	ulong offset = 0;

	int t = 13;			// размер блока в битах
	int len;			// количество блоков
	len = srclen * 8;
	if (len % t == 0)
		len = len / t;
	else
	...
	{
		ulong64 temp0 = des_encrypt_block(T_block, keys);
		//T_block = des_encrypt_block(T_block, keys);

		ulong64_single temp1 = convert_to_ulong64_single(temp0);
		//ulong64_single temp1 = convert_to_ulong64_single(T_block);
		temp1 = (temp1 >> (64 - t)) << (64 - t);	// на всякий случай уберем лишнее

		block = get_block_custom(src, offset, t);	// получаем блок
		block = block << (64 - t);
		block ^= temp1;								// получаем шифроблок
		
		block >>= (64 - t);
		put_block_custom(dst, offset, block, t);	// записываем шифроблок

		//block <<= (64 - t);
		ulong64_single temp2 = convert_to_ulong64_single(T_block);
		temp2 = ((temp2 >> t) << t) + block;
		T_block = convert_to_ulong64(temp2);		// получим блок для след. обработки
		offset += t;
	}
	return 1;
}
...
{
	ulong64_single block = 0;	// обрабатываемый блок 1
	ulong64_single store = 0;	// обрабатываемый блок 1.1
	ulong offset = 0;
	...
	for (ulong i = 0; i < len; i++)
	{
		...
		block <<= (64 - t);
		//if (i == 1) block = change_bit_64(block);
		store = block;

		ulong64_single temp2 = convert_to_ulong64_single(T_block);
		temp2 = ((temp2 >> t) << t) + (store >> (64 - t));
		T_block = convert_to_ulong64(temp2);		// получим блок для след. обработки
		...
		//block <<= (64 - t);
		temp1 ^= block;								// получаем дешифроблок

													//temp1 = (temp1 >> (64 - t)) << (64 - t);
		temp1 >>= (64 - t);
		put_block_custom(dst, offset, temp1, t);	// записываем дешифроблок
...
ulong des_encrypt_ofb(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	for (ulong i = 0; i < len; i++)
	{
		...
		//block = (block >> (64 - t)) << (64 - t);
		block >>= (64 - t);
		put_block_custom(dst, offset, block, t);	// записываем шифроблок

		ulong64_single temp2 = convert_to_ulong64_single(T_block);
		temp2 = ((temp2 >> t) << t) + (temp1 >> (64 - t));
		T_block = convert_to_ulong64(temp2);		// получим блок для след. обработки
		offset += t;
	}
	return 1;
...
ulong des_decrypt_ofb(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	for (ulong i = 0; i < len; i++)
	{
		...
		block <<= (64 - t);
		if (i == 1) block = change_bit_64(block);
		block ^= temp1;								// получаем дешифроблок

		//block = (block >> (64 - t)) << (64 - t);
		block >>= (64 - t);
		put_block_custom(dst, offset, block, t);	// записываем дешифроблок
