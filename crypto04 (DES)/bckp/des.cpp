...
unsigned long long get_block_custom(uchar *src, ulong offset, ulong blocklen)
{
	unsigned long long block;	// 64-битный блок
	ulong bit;					// бит
...
void put_block_custom(uchar *dst, ulong offset, unsigned long long block, ulong blocklen)
{
	offset += blocklen - 1;
	for (ulong i = 0; i < blocklen; i++)
	{
...
{
	int t = 3;			// размер блока в байтах
	int tb = t * 8;		// размер блока в битах
	int len = srclen;	// количество блоков
	if (len % t == 0)
		len = srclen / t;
	else
		len = srclen / t + 1;
	...
	if (t <= 4)
	{
		for (ulong i = 0; i < len; i++)
		{
			...
			ulong temp = T_block.h;
			temp = (temp >> (32 - tb)) << (32 - tb);

			/*uchar sub[8];
			memcpy(sub, &src[i], 5);*/

			GET_ULONG_BE(block.h, src, i * 3);
			block.h = (block.h >> (32 - tb)) << (32 - tb);

			/*GET_ULONG_BE_CUSTOM(block.h, src, 0);
			GET_ULONG_BE_CUSTOM(block.h, src, t);
			GET_ULONG_BE_CUSTOM(block.h, src, 10);*/

			block.h ^= temp;

			T_block.h = (T_block.h << tb) ^ (T_block.l >> (32 - tb));
			T_block.l = (T_block.l << tb) ^ (block.h >> (32 - tb));

			PUT_ULONG_BE(block.h, dst, i * 3);

			//GET_ULONG_BE_CUSTOM(block.h, src, i, 32 - t);
...
ulong des_decrypt_cfb(uchar *src, ulong srclen, uchar *dst, ulong64 key, ulong64 iv)
{
	...
	if (t <= 4)
	{
		for (ulong i = 0; i < len; i++)
		{
			ulong temp = T_block.l;
			if (i != 0)
			{
				T_block.l = (T_block.l << tb) ^ (block.h >> (32 - tb));
				T_block.h = (T_block.h << tb) ^ (temp >> (32 - tb));
			}
			
			T_block = des_encrypt_block(T_block, keys);
			temp = (T_block.h >> (32 - tb)) << (32 - tb);
