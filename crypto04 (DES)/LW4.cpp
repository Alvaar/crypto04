...
int main(int argc, char* argv[])
{
	...
	while (!exit)
	{
		...
		case 3:	// CFB
			...
			cout << "    encrypting: " << msg_enc << endl;
			//msglen = strlen((char*)msg_enc) + 1;		// длина кодированного сообщения
			msg_dec = new uchar[msglen];				// декодированное сообщение
		...
		case 4:	// OFB
			...
			des_encrypt_ofb(msg, msglen, msg_enc, des_key, blk);	// кодирование

			cout << "    encrypting: " << msg_enc << endl;
			//msglen = strlen((char*)msg_enc) + 1;		// длина кодированного сообщения
			msg_dec = new uchar[msglen];				// декодированное сообщение
			des_decrypt_ofb(msg_enc, msglen, msg_dec, des_key, blk);	// декодирование
			cout << "    decrypting: " << msg_dec << endl;
			i = 0;
			system("pause");
			system("CLS");
